...
public class SqliteDBUtil {
    ...
    private static List<File> scanDir(File dir) {
    ...
    public static SqliteDataSource getDaySqliteDatasource(String day, Class cls, boolean createIfNotExist, Consumer<JdbcTemplate> initTable) {
        ...
        File parent = getRootFolder();
        if (day.length() > 4) {
            String p = day.substring(0,day.length()-2);
            parent = new File(parent,p);
            if (!parent.exists()) parent.mkdirs();
        }


        File dbFile = new File(parent,day+".db");
        boolean init = !dbFile.exists();
        if (init && !createIfNotExist) return null;
        SqliteDataSource dataSource = new SqliteDataSource(new File(parent,day+".db"));
